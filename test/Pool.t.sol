...
contract PoolTest is Test {
    ...
    function setUp() external {
        ...
        mine = new Mine();
        erc20Pool = new ERC20Pool(address(usdc),address(mos),212,address(mine),0,0,0,nationalTreasury,10*10**18);
        nativePool = new NativePool(address(mos),212,address(mine),0,0,0,payable(nationalTreasury),10*10**18);
    }
    ...
    function testNativeDeposit() public {
        ...
        vm.startPrank(bob);
        nativePool.deposit{value: 100 * 10 ** 18}(100 * 10 ** 18);
        vm.stopPrank();
